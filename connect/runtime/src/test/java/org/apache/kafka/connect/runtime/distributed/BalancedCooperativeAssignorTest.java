...
public class BalancedCooperativeAssignorTest {
    ...
    /**
     * Test that worker join delay prevents premature rebalancing when workers join in succession.
     * This simulates an autoscaling scenario where multiple workers join within a short time window.
     */
    @Test
    public void testWorkerJoinDelayPreventsMultipleRebalances() {
        // Setup assignor with 30 second worker join delay
        int workerJoinDelayMs = 30000;
        assignor = new BalancedCooperativeAssignor(new LogContext(), time, REBALANCE_DELAY, workerJoinDelayMs);
        assignor.previousGenerationId = generationId;
        
        // Setup connectors with tasks
        addNewConnector("connector-1", 10);
        addNewConnector("connector-2", 10);
        
        // Initial state: 2 workers with assignments
        addNewEmptyWorkers("worker1", "worker2");
        performStandardRebalance();
        
        int tasksOnWorker1 = returnedAssignments.allAssignedTasks().get("worker1").size();
        int tasksOnWorker2 = returnedAssignments.allAssignedTasks().get("worker2").size();
        
        assertTrue("Worker1 should have tasks", tasksOnWorker1 > 0);
        assertTrue("Worker2 should have tasks", tasksOnWorker2 > 0);
        assertEquals("All tasks should be assigned", 20, tasksOnWorker1 + tasksOnWorker2);
        
        // Worker 3 joins at time T1
        time.sleep(1000); // Advance time
        addNewEmptyWorkers("worker3");
        performRebalanceWithDelay();
        
        // Verify that no rebalance occurred (worker3 should have no tasks)
        int tasksOnWorker3 = returnedAssignments.allAssignedTasks().getOrDefault("worker3", Collections.emptyList()).size();
        assertEquals("Worker3 should have no tasks during delay", 0, tasksOnWorker3);
        assertEquals("Worker1 tasks should be unchanged", tasksOnWorker1, 
                returnedAssignments.allAssignedTasks().get("worker1").size());
        assertEquals("Worker2 tasks should be unchanged", tasksOnWorker2,
                returnedAssignments.allAssignedTasks().get("worker2").size());
        
        // Worker 4 joins at time T1 + 10s (within delay window)
        time.sleep(10000);
        addNewEmptyWorkers("worker4");
        performRebalanceWithDelay();
        
        // Verify that still no rebalance occurred
        int tasksOnWorker4 = returnedAssignments.allAssignedTasks().getOrDefault("worker4", Collections.emptyList()).size();
        assertEquals("Worker4 should have no tasks during delay", 0, tasksOnWorker4);
        assertEquals("Worker3 should still have no tasks during delay", 0,
                returnedAssignments.allAssignedTasks().getOrDefault("worker3", Collections.emptyList()).size());
        
        // Advance time past the delay (T1 + 31s)
        time.sleep(21000);
        performRebalanceWithDelay();
        
        // Verify that rebalance occurred and tasks are distributed across all 4 workers
        int finalTasksW1 = returnedAssignments.allAssignedTasks().get("worker1").size();
        int finalTasksW2 = returnedAssignments.allAssignedTasks().get("worker2").size();
        int finalTasksW3 = returnedAssignments.allAssignedTasks().get("worker3").size();
        int finalTasksW4 = returnedAssignments.allAssignedTasks().get("worker4").size();
        
        assertTrue("Worker3 should have tasks after delay", finalTasksW3 > 0);
        assertTrue("Worker4 should have tasks after delay", finalTasksW4 > 0);
        assertEquals("All tasks should still be assigned", 20, 
                finalTasksW1 + finalTasksW2 + finalTasksW3 + finalTasksW4);
        
        // Verify balanced distribution (each worker should have ~5 tasks)
        assertTrue("Tasks should be balanced", Math.abs(finalTasksW1 - 5) <= 1);
        assertTrue("Tasks should be balanced", Math.abs(finalTasksW2 - 5) <= 1);
        assertTrue("Tasks should be balanced", Math.abs(finalTasksW3 - 5) <= 1);
        assertTrue("Tasks should be balanced", Math.abs(finalTasksW4 - 5) <= 1);
    }
    
    /**
     * Test that worker join delay is cancelled if a worker leaves during the delay period.
     * This ensures fault tolerance is not compromised by the delay mechanism.
     */
    @Test
    public void testWorkerJoinDelayCancelledOnWorkerDeparture() {
        // Setup assignor with 30 second worker join delay
        int workerJoinDelayMs = 30000;
        assignor = new BalancedCooperativeAssignor(new LogContext(), time, REBALANCE_DELAY, workerJoinDelayMs);
        assignor.previousGenerationId = generationId;
        ...
        // Initial state: 2 workers
        addNewEmptyWorkers("worker1", "worker2");
        performStandardRebalance();
        
        // Worker 3 joins, starting delay
        time.sleep(1000);
        addNewEmptyWorkers("worker3");
        performRebalanceWithDelay();
        
        // Verify delay is active (worker3 has no tasks)
        assertEquals("Worker3 should have no tasks during delay", 0,
                returnedAssignments.allAssignedTasks().getOrDefault("worker3", Collections.emptyList()).size());
        
        // Worker 1 leaves during delay (at T1 + 10s)
        time.sleep(10000);
        removeWorkers("worker1");
        performRebalanceWithDelay();
        
        // Verify that delay was cancelled and rebalance occurred immediately
        // Worker3 should now have tasks (from worker1's departed tasks)
        int tasksOnWorker3 = returnedAssignments.allAssignedTasks().get("worker3").size();
        assertTrue("Worker3 should have tasks after worker1 departed", tasksOnWorker3 > 0);
        
        // Verify all tasks are still assigned
        int totalTasks = returnedAssignments.allAssignedTasks().values().stream()
                .mapToInt(Collection::size)
                .sum();
        assertEquals("All tasks should still be assigned after worker departure", 10, totalTasks);
    }
    
    /**
     * Test that worker join delay is disabled when workerJoinDelayMs is 0 (default behavior).
     */
    @Test
    public void testWorkerJoinDelayDisabledByDefault() {
        // Use default constructor (workerJoinDelayMs = 0)
        assignor = new BalancedCooperativeAssignor(new LogContext(), time, REBALANCE_DELAY);
        assignor.previousGenerationId = generationId;
        ...
        // Verify that rebalance occurred immediately (worker3 has tasks)
        int tasksOnWorker3 = returnedAssignments.allAssignedTasks().get("worker3").size();
        assertTrue("Worker3 should have tasks immediately when delay is disabled", tasksOnWorker3 > 0);
    }
    
    private void performRebalanceWithDelay() {
        Map<String, ConnectorsAndTasks> memberAssignmentsCopy = memberAssignments.entrySet().stream()
                .collect(Collectors.toMap(
                        Map.Entry::getKey,
                        e -> new ConnectorsAndTasks.Builder()
